function [loglikelihood, Qvalues, proba, RPE, trial_logL, simulated_actions, simulated_rwds] = SimulateQL(data, alpha, beta, constrained, reset)
...
for t=1:n_trials
    ...
    if t < n_trials
        if reset && data(t+1,2) ~= data(t,2) % reset Q values
            ...
            Qvalues(t+1, :) = Qvalues(t, :) + alpha * RPE(t,:);
        end
    end
end
